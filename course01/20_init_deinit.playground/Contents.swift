...
// 프로퍼티 기본 값을 ㄹ지정하기 어려운 경우
// 이니셜라이저를 통해 인스턴스가 가져야 할 초기값을 전달할 수 있음
...
class PersonD {
    ...
    init?(name: String, age: Int) {
        ...
        if name.count == 0 { // name.characters.count -> name.count  -> Swift 5 이후 버전
            return nil
        }
...
donald?.pet = happy
donald = nil // donald 인스턴ㅅ느가 더이상 필요없으므로 메모리에서 해제
